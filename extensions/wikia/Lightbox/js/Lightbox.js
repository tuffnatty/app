...
(function (window, $) {
	...
	var Lightbox = {
		...
		makeLightbox: function (params) {
			...
			Lightbox.includeLatestPhotos = !$('#LatestPhotosModule .carousel-container')
				.length;
			...
			Lightbox.shortScreen = $(window)
				.height() < LightboxLoader.defaults.height + LightboxLoader.defaults.topOffset ? true : false;
			...
			LightboxTracker.track(Wikia.Tracker.ACTIONS.IMPRESSION, '', Lightbox.current.placeholderIdx, {
				title: trackingTitle,
				'carousel-type': trackingCarouselType
			});
			...
			Lightbox.openModal.on('mousemove.Lightbox', function (evt) {
				...
				.on('click.Lightbox', '.LightboxHeader .share-button', function (evt) {
					...
					Lightbox.getShareCodes({
						...
						var trackingTitle = Lightbox.current.key;
						LightboxTracker.track(Wikia.Tracker.ACTIONS.CLICK, 'lightboxShare', null, {
							title: trackingTitle,
						...
						Lightbox.openModal.share.find('.social-links')
							.on('click', 'a', function () {
								var shareType = $(this)
									.attr('class');
								LightboxTracker.track(Wikia.Tracker.ACTIONS.SHARE, shareType, null, {
									title: trackingTitle,
									type: Lightbox.current.type
								});
							});
			...
			updateLightbox: function (data) {
				Lightbox.image.getDimensions(data.imageUrl, function (dimensions) {
					...
					Lightbox.image.trackingTimeout = setTimeout(function () {
						...
						LightboxTracker.track(Wikia.Tracker.ACTIONS.VIEW, 'image', Lightbox.openModal.aggregateViewCount, {
							title: trackingTitle,
		...
		video: {
			...
			updateLightbox: function (data) {
				...
				Lightbox.video.trackingTimeout = setTimeout(function () {
					Lightbox.openModal.aggregateViewCount++;
					LightboxTracker.track(Wikia.Tracker.ACTIONS.VIEW, 'video', Lightbox.openModal.aggregateViewCount, {
						title: trackingTitle,
			...
			function doShareEmail(addresses) {
				$.nirvana.sendRequest({
					...
					callback: function (data) {
						...
						var trackingTitle = Lightbox.current.key; // prevent race conditions from timeout
						LightboxTracker.track(Wikia.Tracker.ACTIONS.SHARE, 'email', null, {
							title: trackingTitle,
							type: Lightbox.current.type
						});
					}
			...
			articleMedia: function (backfill) {
				...
				if (cached.length) {
					...
					thumbs.each(function () {
						...
						var videoName = $thisThumb.attr('data-video-name') || $thisThumb.parent()
